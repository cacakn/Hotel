  ...
  <div>
    ...
    <el-card>
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="4">
          <el-button type="primary" @click="addDialogVisible = true">添加商品订单</el-button>
        </el-col>
      </el-row>
      ...
      <el-table :data="orderlist" border stripe>
        ...
        <el-table-column label="订单数量" prop="productNum"></el-table-column>
        <el-table-column label="订单单价" prop="productPrice"></el-table-column>
        <el-table-column
          label="是否完成"
    ...
    <!-- 添加商品订单的对话框 -->
    <el-dialog title="添加商品订单" :visible.sync="addDialogVisible" width="50%" @close="addDialogClosed">
      <!-- 内容主体区域 -->
  ...
  methods: {
    async getOrderList() {
      const { data: res } = await this.$http.get(
        "http://101.132.135.179:8088/hotel/select/getUserProductList",
        {
          params: this.queryInfo
        }
      );
      if (res.code != 0) {
        return this.$message.error("获取商品订单列表失败！");
      }
      this.$message.success("获取商品订单列表成功！");

      console.log("商品订单列表", res);
    ...
    addOrder() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        const { data: res } = this.$http
          .post(
            "http://101.132.135.179:8088/hotel/userProduct/insertUserProduct?" +
              "userId=" +
              this.addForm.userId +
              "&productId=" +
          ...
          .then(res => {
            console.log("添加商品订单", res);
            if (res.data.code == 0) {
              this.$message.success("添加商品订单成功！");
    ...
    async removeOrderById(id) {
      ...
      const { data: demo } = await this.axios.post(
        "http://101.132.135.179:8088/hotel/select/getUserProductById?id=" + id
      );

      console.log("根据id查询数据", demo);
      if (demo.code != 0) {
        return this.$message.success("查询商品订单信息失败！");
      }
      this.$message.success("查询商品订单信息成功！");
      this.editForm = demo.data;
      console.log("商品订单数据", demo.data);
      ...
      const { data: res } = await this.axios
        .post(
          "http://101.132.135.179:8088/hotel/userProduct/deleteUserProduct?id=" +
            id +
            "&productId=" +
        ...
        .then(res => {
          console.log("res", res);
          if (res.code != 0) {
            return this.$message.success("删除商品订单成功！");
          }
          this.$message.error("删除商品订单失败！");
        });
      this.getOrderList();
    }
  }
